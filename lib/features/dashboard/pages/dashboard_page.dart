  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: dataStream.when(
          ...
          data: (data) {
            var doctors = data.doctors.toList();
            var doctorsByCretedAt = groupBy(
                doctors,
                (obj) => DateFormat('dd/MM/yy').format(
                    DateTime.fromMillisecondsSinceEpoch(obj.createdAt!)));
            var doctorsByCretedAtToEntries = doctorsByCretedAt.keys
                .toList()
                .map((e) => {'date': e, 'count': doctorsByCretedAt[e]!.length})
                .toList();

            var doctorsBySpeciality = groupBy(
                doctors, (obj) => obj.userMetaData!['doctorSpeciality']);
            var doctorsBySpecialityToEntries = doctorsBySpeciality.keys
                .toList()
                .map(
                    (e) => {'date': e, 'count': doctorsBySpeciality[e]!.length})
                .toList();
            ...
            var appointmentsByDateToEntries = appointmentsByDate.keys
                ...
                var patients = data.patients.toList();
            ...
            return Column(
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    GraphItel(data:doctorsByCretedAtToEntries.length > 12
                        ? doctorsByCretedAtToEntries.sublist(0, 11)
                        : doctorsByCretedAtToEntries,title: 'Doctors By Joining Date',gradientColors: [
                      Colors.blue,
                    ...
                    GraphItel(data:
                      doctorsBySpecialityToEntries.length > 9
                    ...
                    GraphItel(data:appointmentsByDateToEntries.length > 12
                        ? appointmentsByDateToEntries.sublist(0, 11)
                        : appointmentsByDateToEntries,title: 'Appointments By Date',gradientColors: [
                      Colors.orange,
                      Colors.orange.withOpacity(.5),
                      Colors.orange.withOpacity(.2),
                    ],),

                    GraphItel(data:patientsByDateToEntries.length > 12
                        ? patientsByDateToEntries.sublist(0, 11)
                        : patientsByDateToEntries,title: 'Patients By Date',gradientColors: [
                      Colors.purple,
